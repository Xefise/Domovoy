...
function CreateApartmentPage(props: Props) {
    ...
    return  <motion.div variants={upVariants} initial={'init'} animate={'show'} exit={'hide'} className={'layout'}>
        ...
        <button onClick={() => {
            if (house && entrance) ConstructionCompanyService.postApiConstructioncompanyApartaments({area, floor, apartmentNumber, isSelling, houseEntranceId: parseInt(entrance)}).finally(() => {
